...
if ($twitter->response['code'] == 200) {
    ...
    foreach ($data as $tweet) {
        ...
        if (strtolower(substr($tweet->text, 0, strlen($command))) == strtolower($command)) {
            ...
            $db->query("INSERT INTO `tweets` (`date`,`tweet_id`,`screen_name`,`gamertag`,`attempts`,`status`) VALUES ('".date('Y-m-d H:i:s')."','".$db->escape($tweet->id_str)."','".$db->escape($tweet->user->screen_name)."','".$db->escape($gamertag)."','0','0')");
        }
        $i++;
    }
    if ($i > 0) {
